...
RooRealVar l2_ProbNNe("l2_ProbNNe","l2_ProbNNe",0.,1.);
//RooRealVar l1_ProbNNmu("l2_ProbNNmu","l2_ProbNNmu",0.,1.);
RooRealVar h1_ProbNNk("h1_ProbNNk","h1_ProbNNk",0.,1.);
...
/////////Get data////////////////////
 ...
 TString dir="/opt/ppd/lhcb/sds61239/mva/addMva/sPlots/"+decay+"/";//probNNeCut/";
...
//////Make model/////////////////
  ...
  //double invMass=5279.61;
   RooRealVar mB("mB", "Bs Mass", 5250., lowRange, highRange);
  ...
  RooRealVar sigma2("sigma2", "Width of Gaussian2", 30,10.,100,"GeV");
  // RooGaussian mBModel2("Gauss2", "Gaussian2", B_MM, mB2, sigma2);
  RooRealVar mean("mean", "mean", 5250., 5000, 5400, "MeV"); 
  //RooRealVar sigma("sigma", "sigma", 5, 0., 1000., "MeV"); 
  RooRealVar alphaCB("alphaCB", "alpha CB", 1., 0.5, 10.); 
  RooRealVar nCB("nCB", "n CB", 1., 0., 999.); 
  //RooRealVar sigma2("sigma2", "sigma CB2", 20, 0., 1000., "MeV"); 
  RooRealVar alphaCB2("alphaCB2", "alpha CB2", -1.,-10., -0.5); 
  ...
  RooRealVar sigfrac("sig1frac","fraction of component 1 in signal",0.05, 0, 1);
...
TCanvas *c22 = new TCanvas;
 ...
 MakePlot(data, data_unchanged, mc, MVAlowerJ, "MVAlowerJ", ndata, nmc,.11);
...
c21->Print(dir+decay+"-"+"alphaHOP.png");
 ...
 MakeResPlot(data, mc, MVAJ16var, "MVAJ16var_res", ndata, nmc);

 c25->Print(dir+decay+"-"+"MVApartJ16var_res.eps");
 c25->Print(dir+decay+"-"+"MVApartJ16var_res.png");

 TCanvas *c26 = new TCanvas;
 
 MakeResPlot(data, mc, MVAJ, "MVAJ_res", ndata, nmc);

 c26->Print(dir+decay+"-"+"MVApartJ_res.eps");
 c26->Print(dir+decay+"-"+"MVApartJ_res.png");
...
void MakeResPlot(RooDataSet *data, RooDataSet *mc, RooRealVar control, TString control_string,
	      float ndata, float nmc)
{
  ...
  RooDataHist *histw_b =dataw_b2->binnedClone();
  RooHistPdf *histw_b2 = new RooHistPdf("histw_b2","histw_b2",control, *histw_b,intorder);
  TH1 *hData=histw_b2->createHistogram("hData",control);
  ...
  TH1 *hMC=histmc2->createHistogram("hMC",control);
  hMC->SetTitle("MC-sWeighted data");
  gStyle->SetOptStat(0);
  hMC->Add(hData,-1.);
